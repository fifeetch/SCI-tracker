...
function applyEntityUI(){
  ...
  if(logo && !logo.classList.contains('app-logo-mark')) logo.textContent = 'SF';
...
function defaultPushPreferences(){
  return {message:true,reunion:true,decision:true,document:true,alert:true};
}
function currentPushPreferences(){
  const uid=auth.currentUser?.uid || '';
  const token=(window.CACHE?.pushTokens||[]).find(t=>String(t.uid||'')===uid && t.enabled!==false);
  return {...defaultPushPreferences(), ...(token?.preferences||{})};
}
function readPushPreferencesFromUI(){
  return {
    message: !!$('push-pref-message')?.checked,
    reunion: !!$('push-pref-reunion')?.checked,
    decision: !!$('push-pref-decision')?.checked,
    document: !!$('push-pref-document')?.checked,
    alert: true
  };
}
...
function updatePushNotificationSettingsUI(){
  ...
  const prefIds={message:'push-pref-message',reunion:'push-pref-reunion',decision:'push-pref-decision',document:'push-pref-document'};
  Object.entries(prefIds).forEach(([key,id])=>{
    const el=$(id);
...
async function enablePushNotifications(){
  ...
  try{
    ...
    const payload={
      ...
      token,
      enabled:true,
      preferences:readPushPreferencesFromUI(),
...
async function savePushPreferences(){
  if(!canWrite()){ denyWrite(); return; }
  const prefs=readPushPreferencesFromUI();
  try{
    const token=await currentPushToken();
    if(!token){
      toast('Active d abord cet appareil pour enregistrer ses choix.');
      return;
    }
    const id=pushTokenDocId(token);
    await colRef('pushTokens').doc(id).set({
      id,
      uid:auth.currentUser?.uid||'',
      email:auth.currentUser?.email||'',
      role:APP_STATE.role,
    ...
    if(Array.isArray(window.CACHE.pushTokens)){
      const idx=window.CACHE.pushTokens.findIndex(x=>String(x.id)===id);
      if(idx>=0) window.CACHE.pushTokens[idx]={...window.CACHE.pushTokens[idx],preferences:prefs,enabled:true};
    }
    updatePushNotificationSettingsUI();
    toast('Choix de notifications enregistres ✓');
  }catch(err){ console.error(err); toast(formatFirebaseError(err)); }
}
...
async function genQuittance(data){
  ...
  const logoUrl=new URL('icons/icon.svg',window.location.href).href;
